  ...
  def __init__(self, command: List[str], returncode: int, stderr_path: str):
    super(PipError, self).__init__(
        ...
        'logs at: {stderr_path}'.format(
            command=command, returncode=returncode, stderr_path=stderr_path))
...
class PipCheckResultType(enum.Enum):
  ...
  CHECK_WARNING: Indicates that "pip check" completed with a non-zero exit code.
  """
  SUCCESS = 'success'
...
class PipCheckResult:
  ...
  Attributes:
    ...
    result_text: The text output of "pip install && pip check". For example:
        "Could not find a version that satisfies the requirement tensorflow." or
        "numpy has requirement six<3, but you have six 4.0.1."
        Will be None if "pip install <packages> && pip check" completed without
        error.
    requirements: The text output of "pip freeze" i.e. the packages that were
        installed as a product of the "pip install <packages" command. The
        output can be parsed with pkg_resources.parse_requirements().
  ...
  def __init__(self,
    ...
    requirements: The text output of "pip freeze" i.e. the packages that were
        installed as a product of the "pip install <packages" command.
    """
...
class _OneshotPipCheck():
  ...
  def __init__(self, pip_command: List[str], packages: List[str], tmp_path: str,
               clean: bool):
    ...
    Args:
      ...
      clean: If True then all previously installed packages will be uninstalled
  ...
  def _uninstall(self, requirements_file_path):
    ...
    std_err_path = os.path.join(self._output_directory, 'uninstall-error.txt')
  ...
  def _install(self):
    ...
    if returncode:
      return PipCheckResult(self._packages, PipCheckResultType.INSTALL_ERROR,
  ...
  def _check(self):
    ...
    returncode = self._run_command(
        command, std_out_path, std_err_path, raise_on_failure=False)
    if returncode:
      return PipCheckResult(self._packages, PipCheckResultType.CHECK_WARNING,
