  ...
  for version_mapping in s.split(','):
    try:
      ...
      raise argparse.ArgumentTypeError(
          '{0} is not in the format of <version>:<command>,<version>:<command>'.
          format(s))
  ...
  def _check(self, start_response, python_version, packages):
    ...
    if python_version not in self._python_version_to_command:
      start_response('400 Bad Request',
                     [('Content-Type', 'text/plain; charset=utf-8')])
      return [
          b'Invalid Python version specified. Must be one of: %s' % (', '.join(
    ...
    try:
      ...
      return [
          b'pip command ("%s") ' % pip_error.command_string.encode('utf-8'),
          b'failed with:\n',
...
def main():
  ...
  parser.add_argument(
      '--clean',
      action='store_true',
      help='uninstall existing packages before performing dependency checking')
  parser.add_argument(
      '--install-once',
      action='store_true',
  ...
  parser.add_argument(
      ...
      help='maps version strings to the Python command to execute when running '
